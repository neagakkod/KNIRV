...
class CallerServices {
			...
			if(empty($response) || trim($response) == '')
	   		{
				...
				$errorData->errorId = 'API Error' ;
		  		$errorData->message = 'SOAP response is empty.' ;
	...
	function callWebService($request,$serviceName)
	{
		...
		try {
		    ...
		    $endpoint=API_BASE_ENDPOINT.$serviceName;
		    $response = call($request, $endpoint);
		}
		catch(Exception $ex) {
			throw new FatalException('Error occurred in call method');
		}
...
function call($soapStr, $endpoint, $sandboxEmailAddress)
{
   ...
   //Set proxy name to PROXY_HOST and port number to PROXY_PORT in constants.php
    ...
    //setting the soapStr as POST FIELD to curl
    $conf = array('mode' => 0600, 'timeFormat' => '%X %x');
    $logger = &Log::singleton('file', LOGFILENAME, 'caller', $conf);
    $logger->log("request: $soapStr");

    curl_setopt($ch,CURLOPT_POSTFIELDS,$soapStr);
